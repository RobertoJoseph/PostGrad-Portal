...
async function AddCommentsGrade(ThesisSerialNo,DefenseDate,comments) {
    try {
        let pool = await sql.connect(config);
        const result = (await pool.request().input("ThesisSerialNo", sql.Int, ThesisSerialNo)
        .input("DefenseDate",sql.Date,DefenseDate)
        .input("comments",sql.VarChar,comments)
        .execute(`AddCommentsGrade`)).recordset;
        console.log(result);
        sql.close();
        return result;
...
async function AddDefenseGrade(ThesisSerialNo,DefenseDate,grade) {
    try {
        ...
        .execute(`AddDefenseGrade`)).recordset;
        console.log(result);
        sql.close();
        return result;
...
async function searchThesis(searchkey) {
    try {
        let pool = await sql.connect(config);
        const result = (await pool.request().input("searchkey", sql.VarChar, searchkey).execute(`searchThesis`)).recordset;
        console.log(result);
        sql.close();
        return result;
...
async function examinerAttendedDefense(id) {
    try {
        let pool = await sql.connect(config);
        const result = (await pool.request().input("id", sql.Int, id).execute(`examinerAttendedDefense`)).recordset;
        console.log(result);
        sql.close();
        return result;
